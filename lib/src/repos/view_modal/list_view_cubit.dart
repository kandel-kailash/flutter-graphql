...
class ListViewCubit extends Cubit<ListViewState> {
  ListViewCubit([this.hasNextPage = true])
  ...
  ParsedResult _parseUserResult(Map<String, dynamic> result) {
    ...
    return (owner: _currentOwner!, repos: _repos);
  }
  ...
  Future<void> fetchUserData([User? user]) async {
    ...
    if (result.hasException) {
      emit(ListViewState.failure(result.exception.toString()));

      return;
    }

    final (:User owner, :Set<GithubRepository> repos) = _parseUserResult(
      _currentOwner == null ? result.data!['viewer'] : result.data!['user'],
    );

    emit(ListViewState.loaded(user: owner, repos: repos));
  }
