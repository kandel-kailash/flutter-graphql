...
import 'package:github_graphql_app/src/auth/modal/auth_screen_route_info.dart';
import 'package:github_graphql_app/src/auth/view/widgets/github_logo.dart';
import 'package:github_graphql_app/src/auth/view/widgets/welcome_screen_header.dart';
...
class _SignInButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return FilledButton.icon(
      onPressed: () {
        context.read<AuthCubit>().signIn((authorizationUri) {
          ...
          if (kIsWeb) {
            ...
            //     Exception('Popup blocked. Please allow popups and try again.'),
            //   );
            //
            //   return completer.future;
            // }
            //
            // void eventListener(web.MessageEvent event) {
            //   print('Inside event listener with data: ${event.data}');
            //   print('Even origin: ${event.origin}');
            //
            //   final redirectedUrl = Uri.parse(event.data as String);
            //
            //   popup.close();
            //   completer.complete(redirectedUrl);
            //   popup.removeEventListener('message', eventListener.toJS);
            // }
            //
            // popup.addEventListener('message', eventListener.toJS);
          } else {
            context.goNamed(
      ...
      icon: const Padding(
        ...
        child: GithubLogo(
          size: Size(24, 24),
          colorFilter: ColorFilter.mode(gitHubLogoColor, BlendMode.srcIn),
        ),
      ),
        ...
        child: AnimatedOpacity(
          ...
          child: AnimatedContainer(
            ...
            decoration: BoxDecoration(
              ...
              border: Border.all(color: gitHubLogoColor, width: .1),
                ...
                children: [
                  const Row(
                    ...
                    children: [
                      ...
                      Expanded(child: Center(child: _SignInButton())),
                    ],
                  ),
                  Align(
    ...
    return AnimatedContainer(
      ...
      child: AnimatedFractionallySizedBox(
        ...
        child: AnimatedPadding(
          duration: _controller.duration!,
          padding: EdgeInsets.all((_controller.value * 60) + 40),
          child: AnimatedOpacity(
            duration: _controller.duration!,
            opacity: _controller.value,
            child: const GithubLogo(size: Size.infinite),
          ),
        ),
      ),
    );
